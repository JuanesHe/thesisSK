...
d_actual = P_col - P
v = (d_actual - d_before)/dtime
d_norm = np.sqrt(np.power(d_actual[0,:],2) + np.power(d_actual[1,:],2) + np.power(d_actual[2,:],2) );

urf = d_actual/d_norm
Fd = (-d)*urf*(np.dot(V.reshape(1,3),urf))

Fk = - kd/np.power(d_norm,2)

d_co = Po - P_col
d_ao = Po - P

plan = np.cross(dao,dco)
d_plan = np.sqrt(np.power(plan[0,:],2) + np.power(plan[1,:],2) + np.power(plan[2,:],2) );
...
d_plan = np.sqrt(np.power(d_actual[0,:],2) + np.power(d_actual[1,:],2) + np.power(d_actual[2,:],2) );
plan = plan/ norm(plan);
force = cross(plan ,Fk );
